...
public class BoardCanvas extends Canvas implements Observer {
	...
	class Controller implements PaintListener, MouseMoveListener, MouseTrackListener, MouseListener {
		...
		public void mouseDown(MouseEvent e) {
			...
			int colOrigin = (BoardCanvas.this.xImage - this.boardSettings[BOARD_WIDTH_DELTA_INDEX])
					/ this.boardSettings[SQUARE_WIDTH_INDEX];
			int rowOrigin = (BoardCanvas.this.yImage - this.boardSettings[BOARD_HEIGHT_DELTA_INDEX])
			...
			BoardCanvas.this.possibleNextMoves = BoardCanvas.this.model.allMoves(
					BoardCanvas.this.model.getBoard(BoardCanvas.this.selectedPieceIndex),
					BoardCanvas.this.selectedPieceIndex);
			if (BoardCanvas.this.possibleNextMoves != null) {
				...
				for (int i = 0, max = BoardCanvas.this.possibleNextMoves.length; i < max; i++) {
					...
					BoardCanvas.this.redraw(
							...
							this.boardSettings[BOARD_HEIGHT_DELTA_INDEX]
									+ col * this.boardSettings[SQUARE_HEIGHT_INDEX],
							this.boardSettings[SQUARE_WIDTH_INDEX], this.boardSettings[SQUARE_HEIGHT_INDEX], false);
				}
			}
			updateBuffer();
		}
		...
		public void mouseEnter(MouseEvent e) {
			if (BoardCanvas.this.drag) {
				BoardCanvas.this.xImage = e.x;
				BoardCanvas.this.yImage = e.y;
				BoardCanvas.this.redraw(BoardCanvas.this.xImage, BoardCanvas.this.yImage,
						this.boardSettings[SQUARE_WIDTH_INDEX], this.boardSettings[SQUARE_HEIGHT_INDEX], true);
			}
			updateBuffer();
		}
		...
		public void mouseExit(MouseEvent e) {
			if (BoardCanvas.this.drag) {
				...
				BoardCanvas.this.redraw(oldX, oldY, this.boardSettings[SQUARE_WIDTH_INDEX],
						this.boardSettings[SQUARE_HEIGHT_INDEX], true);
			}
			updateBuffer();
		}
		...
		public void mouseMove(MouseEvent e) {
			if (BoardCanvas.this.drag) {
				BoardCanvas.this.redraw(BoardCanvas.this.xImage + BoardCanvas.this.xOffset,
						BoardCanvas.this.yImage + BoardCanvas.this.yOffset, this.boardSettings[SQUARE_WIDTH_INDEX],
						this.boardSettings[SQUARE_HEIGHT_INDEX], false);
				BoardCanvas.this.xImage = e.x;
				BoardCanvas.this.yImage = e.y;
				updateBuffer();
				BoardCanvas.this.redraw(BoardCanvas.this.xImage + BoardCanvas.this.xOffset,
		...
		public void mouseUp(MouseEvent e) {
			...
			long[] moves = BoardCanvas.this.model
					.allMoves(BoardCanvas.this.model.getBoard(BoardCanvas.this.selectedPieceIndex));
			...
			if (selectedMoveIndex > 0) {
				if (selectedMoveIndex > 1) {
					PromotionDialog dialog = new PromotionDialog(BoardCanvas.this.getShell(),
							BoardCanvas.this.model.getTurn(), this.imageFactory, BoardCanvas.this.model.getLocale());
					dialog.open();
	...
	public BoardCanvas(ChessEngine model, Composite composite, int orientation, ImageFactory imageFactory,
			int[] settings) {
	...
	public void update(Observable o, Object arg) {
		if (o == this.model) {
			this.redraw(this.settings[BOARD_WIDTH_ORIGIN_INDEX], this.settings[BOARD_HEIGHT_ORIGIN_INDEX],
					this.settings[BOARD_WIDTH_INDEX], this.settings[BOARD_HEIGHT_INDEX], true);
			updateBuffer();
		}
	}
	...
	public void updateBuffer() {
		...
		for (int i = 0; i < 4; i++) {
			for (int j = 0; j < 4; j++) {
				gc.drawImage(this.board, 0, 0, this.boardSize.width, this.boardSize.height,
						this.settings[SQUARE_WIDTH_INDEX] * 2 * i, this.settings[SQUARE_HEIGHT_INDEX] * 2 * j,
						this.boardSize.width, this.boardSize.height);
			}
		}
		...
		if (this.possibleNextMoves != null) {
			for (int i = 0, max = this.possibleNextMoves.length; i < max; i++) {
				...
				gc.drawImage(this.possibleMove, 0, 0, this.settings[SQUARE_WIDTH_INDEX],
						...
						this.settings[SQUARE_WIDTH_INDEX], this.settings[SQUARE_HEIGHT_INDEX]);
			}
		}
		// paint pieces
		...
		if (this.drag && this.xImage >= 0 && this.yImage >= 0 && this.dragImage != null) {
			gc.drawImage(this.dragImage, 0, 0, this.settings[SQUARE_WIDTH_INDEX], this.settings[SQUARE_HEIGHT_INDEX],
					this.xImage + this.xOffset, this.yImage + this.yOffset, this.settings[SQUARE_WIDTH_INDEX],
					this.settings[SQUARE_HEIGHT_INDEX]);
		}
		gc.dispose();
	}
